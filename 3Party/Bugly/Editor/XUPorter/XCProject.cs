...
namespace UnityEditor.XCodeEditor
{
	public partial class XCProject : System.IDisposable
	{
		...
		public PBXDictionary AddFile( string filePath, PBXGroup parent = null, string tree = "SOURCE_ROOT", bool createBuildFiles = true, bool weak = false ,bool dontcheckFileRef = false)
		{
			...
			if( fileReference != null && !dontcheckFileRef) {
		...
		public bool AddLocFolder( string folderPath, PBXGroup parent = null, string[] exclude = null, bool createBuildFile = true)
		{
			...
			foreach( string file in Directory.GetFiles( folderPath ) ) {
				...
				AddFile( file, variant, "GROUP", createBuildFile,false,true );
			}
		...
		{	
			...
			if (mod.FrameworkSearchPaths != null)
			{
				foreach (string searchPath in mod.FrameworkSearchPaths)
				{
					this.AddFrameworkSearchPaths(searchPath);
				}
			}
