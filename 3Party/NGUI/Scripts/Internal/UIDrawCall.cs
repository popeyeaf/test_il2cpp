...
//#endif
	...
	void CreateMaterial ()
	{
		...
		{
			mTextureClip = true;
			shader = FindShader ("Hidden/" + shaderName + textureClip);
		} 
		else if (mClipCount != 0) 
		{
			shader = FindShader ("Hidden/" + shaderName + " " + mClipCount);
			if (shader == null) 
			{
				shader = FindShader (shaderName + " " + mClipCount);
			}
			...
			{
				mLegacyShader = true;
				shader = FindShader (shaderName + soft);
			}
		} 
		else 
		{
		...
		if (shader == null) 
		{
			shader = FindShader("Unlit/Transparent Colored");
		}
	...
	Shader FindShader(string shaderName)
	{
		...
		if (Application.isPlaying && NGUIShaderManager.Me != null) 
		{
			resultShader = NGUIShaderManager.Me.GetNGUIShader(shaderName);
		}
		if (resultShader == null) 
		{
			resultShader = Shader.Find(shaderName);
		}
		return resultShader;
	}
