...
//#endif
	...
	static public Vector3[] GetSides (this Camera cam, float depth, Transform relativeTo)
	{
...
#endif
		{
			...
			SafeArea sa = SafeArea.Instance;
			if ( sa != null) 
			{
				x0 = sa.l * 2 / Screen.width - 1;
				x1 = 1 - sa.r * 2 / Screen.width;
				y0 = sa.b * 2 / Screen.height - 1;
				y1 = 1 - sa.t * 2 / Screen.height;
			}
			#endif
		...
		{
			float l = 0f, r = 1f, b = 0f, t = 1f;
			#if UNITY_EDITOR || UNITY_IOS
			SafeArea sa = SafeArea.Instance;
			if (sa != null) 
			{
				l = sa.l / Screen.width;
				r = 1 - (sa.l + sa.r) / Screen.width;
				b = sa.b / Screen.height;
				t = 1 - (sa.b + sa.t) / Screen.height;
			}
			#endif
			mSides[0] = cam.ViewportToWorldPoint(new Vector3(l, 0.5f, depth));
			mSides[1] = cam.ViewportToWorldPoint(new Vector3(0.5f, t, depth));
			mSides[2] = cam.ViewportToWorldPoint(new Vector3(r, 0.5f, depth));
			mSides[3] = cam.ViewportToWorldPoint(new Vector3(0.5f, b, depth));
		}
	...
	static public Vector3[] GetWorldCorners (this Camera cam, float depth, Transform relativeTo)
	{
		...
		else
		{
			...
			#endif
			mSides[0] = cam.ViewportToWorldPoint(new Vector3(l, b, depth));
			mSides[1] = cam.ViewportToWorldPoint(new Vector3(l, t, depth));
			mSides[2] = cam.ViewportToWorldPoint(new Vector3(r, t, depth));
			mSides[3] = cam.ViewportToWorldPoint(new Vector3(r, b, depth));
		}
