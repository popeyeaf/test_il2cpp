...
public class UITexturePacker
{
	...
	public static bool FitMaxSize(List<string> handled, Texture2D[] textures, int width, int height, int padding, int maxSize)
	{
		...
		for (int i = 0; i < textures.Length; i++)
		{
			...
			for (xPadding = 1; xPadding >= 0; --xPadding)
			{
				for (yPadding = 1; yPadding >= 0; --yPadding)
				{
					...
					{
						return FitMaxSize(handled,textures, width * (width <= height ? 2 : 1),
						                    height * (height < width ? 2 : 1), padding, maxSize);
					}
				}
				if (rect.width != 0 && rect.height != 0) break;
			...
			storage[i].paddingX = (xPadding != 0);
			storage[i].paddingY = (yPadding != 0);
			if(handled.Contains(tex.name)==false)
			{
				handled.Add(tex.name);
			}
		}
