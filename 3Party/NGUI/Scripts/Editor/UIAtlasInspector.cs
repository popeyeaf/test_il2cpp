	...
	public override void OnInspectorGUI ()
	{
		...
		if (mAtlas.spriteMaterial != null)
		{
			...
			if (sprite != null)
			{
				...
				if(GUILayout.Button("检测原图是否丢失"))
				{
					string path = EditorUtility.SaveFolderPanel ("选择图片元素路径","Assets/Art/Public/Texture/GUI/Icon","");
					if(!string.IsNullOrEmpty(path))
					{
						string[] pics = Directory.GetFiles (path, "*.*", SearchOption.AllDirectories);
						Dictionary<string,bool> picSource = new Dictionary<string, bool>();
						string fileName = null;
						string extension = null;
						for(int i=0;i<pics.Length;i++)
						{
							extension = Path.GetExtension(pics[i]);
							if(extension == ".png" || extension == ".jpg")
							{
								fileName = Path.GetFileNameWithoutExtension(pics[i]);
								picSource.Add(fileName,true);
							}
						}
						UISpriteData uiSpriteData = null;
						System.Text.StringBuilder sb = new System.Text.StringBuilder();
						sb.AppendLine(string.Format("{0} 在资源目录下找不到以下元素名:",mAtlas.name));
						for(int j=0;j<mAtlas.spriteList.Count;j++)
						{
							uiSpriteData = mAtlas.spriteList[j];
							if(picSource.ContainsKey(uiSpriteData.name)==false)
							{
								sb.AppendLine(string.Format("\t\t{0}",uiSpriteData.name));
							}
						}
