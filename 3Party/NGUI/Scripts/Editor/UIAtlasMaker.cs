...
public class UIAtlasMaker : EditorWindow
{
	...
	static bool PackTextures (Texture2D tex, List<SpriteEntry> sprites,int max = 0)
	{
		...
		int maxSize = 4096;
#else
		int maxSize = max == 0 ? SystemInfo.maxTextureSize : max;
#endif

#if UNITY_ANDROID || UNITY_IPHONE
		max = Mathf.Min(max,NGUISettings.allow4096 ? 4096 : 2048);
		if(max!=0)
		{
			maxSize = Mathf.Min(maxSize, max);
		}
#endif
	...
	{
		EditorUtility.DisplayProgressBar("Create Sprites", "please wait...", 0);
		...
		{
			count++;
			EditorUtility.DisplayProgressBar("Create Sprites", string.Format("正在加载图片{0}...", tex.name), count / (float)textures.Count);
	...
	static public bool UpdateTexture (UIAtlas atlas, List<SpriteEntry> sprites,string specifiedPath = "",int maxSize = 0)
	{
		// Get the texture for the atlas
		Texture2D tex = atlas.texture as Texture2D;
		string oldPath = (tex != null) ? AssetDatabase.GetAssetPath(tex.GetInstanceID()) : "";
		string newPath = NGUIEditorTools.GetSaveableTexturePath(atlas,specifiedPath);
		...
		if (PackTextures(tex, sprites,maxSize))
		{
	...
	static public void UpdateAtlas (List<Texture> textures, bool keepSprites,string specifiedPath = "",int maxSize = 0)
	{
		...
		if (sprites.Count > 0)
		{
			...
			// Update the atlas
			UpdateAtlas(NGUISettings.atlas, sprites,specifiedPath,maxSize);
		}
		else if (!keepSprites)
		{
			UpdateAtlas(NGUISettings.atlas, sprites,specifiedPath,maxSize);
	...
	static public void UpdateAtlas (UIAtlas atlas, List<SpriteEntry> sprites,string specifiedPath = "",int maxSize = 0)
	{
		if (atlas == null) {
			atlas = NGUISettings.atlas;
		}
		if (sprites.Count > 0)
		{
			...
			if (UpdateTexture(atlas, sprites,specifiedPath,maxSize))
			{
				// Replace the sprites within the atlas
